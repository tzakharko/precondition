...
diagnose_failed_conditions <- function(default_message, .env, embraced_exprs, ...) {
  ...
  for(i in idx) {
    ...
    # get the location message
    assertion_message <- rev(...names()[seq_len(i)])
    assertion_message <- (assertion_message[assertion_message != ""])[[1L]]
    if(is.null(assertion_message)) assertion_message <- default_message
